...
EcomGraphAI 是一个面向电商客服场景的知识图谱增强问答系统。项目基于 Neo4j 构建电商知识图谱，并结合 Graph RAG 流程，将 MySQL 业务数据库、商品详情图片 OCR 结果与用户行为日志三类数据来源接入问答链路，支持商品属性查询、多条件筛选、用户兴趣查询以及多轮上下文追问等对话场景。

图谱构建阶段，系统首先从 MySQL 业务数据库中读取 SKU、SPU、一级/二级/三级品类、品牌和商品属性等结构化数据，将其抽象为 SKU、SPU、Category、Trademark、Attr 等节点，并构建 Belong、Have 等关系；随后从商品详情图片中识别文字并抽取商品属性，以增量方式补充 Attr 节点和 Have 关系；最后接入用户浏览、点击、收藏日志，构建 User 节点以及 View、Click、Favorite 等用户行为关系，用于刻画用户兴趣偏好。

在问答阶段，系统基于 Graph RAG 实现从自然语言到图谱查询的完整流程，包括上下文记忆读取、查询纠错、上下文问题改写、意图识别、实体抽取、混合检索、入口节点过滤、Cypher 生成与校验、Neo4j 查询和回答生成等步骤，从而完成面向电商客服的自然语言检索与问答。
