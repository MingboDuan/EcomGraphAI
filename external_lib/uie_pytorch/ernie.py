...
# transformers>=5.0 移除了部分工具函数，兼容处理
try:
...
class ErnieConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # transformers>=5.0 不再为缺失属性提供默认值，手动补齐
        self.is_decoder = kwargs.get('is_decoder', False)
        self.cross_attention_hidden_size = kwargs.get('cross_attention_hidden_size', None)
...
class ErnieEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if position_ids is None:
            # transformers>=5.0 meta device 初始化可能导致 position_ids buffer 值异常，直接用 arange 生成
        ...
        # issue #5664
        _device = input_ids.device if input_ids is not None else inputs_embeds.device
        if token_type_ids is None:
            token_type_ids = torch.zeros(
                input_shape, dtype=torch.long, device=_device)
        ...
        if self.use_task_id:
            if task_type_ids is None:
                task_type_ids = torch.ones(
                    input_shape, dtype=torch.long, device=_device) * self.task_id
...
class ErnieModel(ErniePreTrainedModel):
    ...
    def get_head_mask(self, head_mask, num_hidden_layers, is_attention_chunked=False):
        """兼容 transformers>=5.0（该方法已从 PreTrainedModel 移除）"""
        if head_mask is not None:
    ...
    def forward(
        ...
        # transformers>=5.0: get_extended_attention_mask 签名变为 (mask, input_shape, dtype=...)
        try:
